...
import { getPathVal, setPathVal, deletePathVal, nodePath2ClassName } from '@/utils/vue3Utils'
import { isEmptyObject, filterObject, isObject, getSchemaType } from '@/utils/utils'

import { getWidgetConfig, getUiOptions, getUserErrOptions } from '@/utils/formUtils'
...
export default {
  ...
  setup(props) {
    ...
    const curSelectIndex = ref(
      computedCurSelectIndexByFormData(getPathVal(props.rootFormData, props.curNodePath))
    )
    ...
    watch(curSelectIndex, (newVal, oldVal) => {
      ...
      // 计算出 新选项默认值
      const newOptionData = getDefaultFormState(
      ...
      if (isObject(curFormData)) {
        const oldSelectSchema = retrieveSchema(props.selectList[oldVal], props.rootSchema)
    ...
    return () => {
      ...
      // object类型但没有附加属性
      const isObjectEmptyAttachProperties =
        isTypeObject && isEmptyObject(curSelectSchema && curSelectSchema.properties)

      if (curSelectSchema && !isObjectEmptyAttachProperties) {
        // 当前节点的ui err配置，用来支持所有选项的统一配置
        // 取出 oneOf anyOf 同级配置，然后再合并到 当前选中的schema中
        const userUiOptions = filterObject(
          getUiOptions({
            schema: props.schema,
        ...
        const userErrOptions = filterObject(
          getUserErrOptions({
            schema: props.schema,
            uiSchema: props.uiSchema,
