...
import { isObject, deepEquals } from '../utils'
...
export function validateFormDataAndTransformMsg({
  ...
  if (filterRootNodeError) {
    ajvErrors = ajvErrors.filter(
      (item) =>
        (item.property === '' &&
          !item.schemaPath.includes('#/anyOf/') &&
          !item.schemaPath.includes('#/oneOf/')) ||
  ...
  return (isOnlyFirstError && ajvErrors.length > 0 ? [ajvErrors[0]] : ajvErrors).reduce(
    (preErrors, errorItem) => {
      // 优先获取 errorSchema 配置
      errorItem.message =
        userErrOptions[errorItem.name] !== undefined
          ? userErrOptions[errorItem.name]
          : errorItem.message
...
export function getMatchingIndex(formData, options, rootSchema, haveAllFields = false) {
  ...
  if (options[0] && options[0].properties) {
    const constProperty = Object.keys(options[0].properties).find(
      (k) => options[0].properties[k].const
    )
    if (constProperty) {
      // eslint-disable-next-line no-plusplus
      for (let i = 0; i < options.length; i++) {
