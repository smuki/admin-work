...
import { IconQuestion } from '@/utils/icons'

import { validateFormDataAndTransformMsg } from '@/utils/schema/validate'
import { fallbackLabel } from '@/utils/formUtils'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    return () => {
      ...
      const descriptionVNode = props.description
        ? h('div', {
      ...
      const miniDescriptionVNode =
        miniDesModel && descriptionVNode
          ? h(
              resolveComponent(COMPONENT_MAP.popover),
              {
                style: {
                  margin: '0 2px',
                  fontSize: '16px',
      ...
      console.log('----------------------------------')
      console.log('----------------------------------')
      console.log('genFormProvide')
      console.log('----------------------------------')
      console.log(genFormProvide)
      console.log('----------------------------------')
      console.log('genFormProvide.fallbackLabel')
      console.log('----------------------------------')
      console.log(genFormProvide.fallbackLabel)
      // 运行配置回退到 属性名
      const label = fallbackLabel(
        props.label,
        props.widget && genFormProvide.fallbackLabel.value,
        props.curNodePath
      )
      return h(
              ...
              ? [
                  h(
                    ...
                    resolveComponent(props.widget),
                    {
                      ...
                      ...(otherAttrs
                        ? (() =>
                            Object.keys(otherAttrs).reduce((pre, k) => {
                              pre[k] = otherAttrs[k]

                              // 保证ui配置同名方法 ui方法先执行
                              ;[props.widgetAttrs[k], props.uiProps[k]].forEach((uiConfFn) => {
                    ...
                    },
                    {
                      ...(props.renderScopedSlots
                        ? typeof props.renderScopedSlots === 'function'
                          ? props.renderScopedSlots()
                          : props.renderScopedSlots
                        : {}),
                    }
