...
import { computedCurPath, getPathVal } from '@/utils/vue3Utils'
import { isObject } from '@/utils/utils'
import FieldGroupWrap from '@/utils/components/FieldGroupWrap.vue'
...
export default {
  ...
  setup(props) {
    ...
    const isRequired = (name) =>
      Array.isArray(props.schema.required) && !!~props.schema.required.indexOf(name)
    ...
    const isDependOn = (name) => {
      ...
      if (isObject(props.schema.dependencies)) {
        curDependent = Object.entries(props.schema.dependencies).some(([key, value]) => {
          ...
          // 当前需要依赖
          return (
            tempDependency && getPathVal(props.rootFormData, props.curNodePath)[key] !== undefined
          )
        })
      }
    ...
    return () => {
      ...
      const propertiesVNodeList = orderedProperties.map((name) => {
        ...
        return isDependency && onlyShowIfDependent && !curDependent
          ? null
          : h(SchemaField, {
              key: name,
              ...props,
