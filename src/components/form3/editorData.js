...
import { isObject, isEmptyObject } from '@/utils/utils'
...
export function generateEditorItem(toolItem) {
  ...
  const ids = [
    currentComponentPack.viewSchema.format,
    currentComponentPack.viewSchema.type,
    genId(),
  ]
  ...
  return {
    ...
    id,
    ...(currentComponentPack.viewSchema.properties ||
    (currentComponentPack.viewSchema.items && currentComponentPack.viewSchema.items.properties)
      ? { childList: [] }
      : {}),
  }
}
...
function filterObj(
  obj,
  filter = (key, value) => (isObject(value) && !isEmptyObject(value)) || value !== undefined
) {
  const result = {}
...
export function componentList2JsonSchema(componentList) {
  ...
  while (queue.length) {
    ...
    if (item.$$parentFlag) {
      ...
      // 连接数据
      ;(parentObj.properties || parentObj.items.properties)[item.componentValue.property] =
        curSchema
